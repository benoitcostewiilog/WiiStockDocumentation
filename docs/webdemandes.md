...
Sur le menu demande de collecte, cliquez sur le bouton `Nouvelle collecte`. Cela ouvrira une fenêtre de création d'un entête de demande de collecte : 
...
Vous venez de créer un entête de demande de collecte.
...
Dans le cas où vous ne connaitriez pas le code de la référence, vous pouvez cliquer sur le bouton `Chercher une référence`, cela vous redirigera vers le référentiel des articles de référence. 
...
Date de validation | Permet de savoir quand la demande a été validée.
...
Pour inspecter une référence, il faut cliquer sur celle-ci. 
...
Demandeur| Indique le nom d'utilisateur du demandeur. Ce champ est prérempli en fonction de l'utilisateur connecté sur l'application.
